...
;(async function() {
  ...
  try {
    ...
    for(let i = 8; i < pageNum; i++) {
      eventloop.add(() => { return pageList(i+1) })
    }
    eventloop.loop()
    ...
    async function pageList(num) {
      ...
      arrPage.forEach(async (pageObj, i) => {
        // await cbWait(4000 * i)
        eventloop.addTop(() => { return getPageInfo(pageObj) })
        // getPageInfo(pageObj)
      })
      // eventloop.loop()
      return true
    ...
    async function getPageInfo(pageObj) {
      ...
      let eleList = await page.$$eval('.bookinfo > ul > li', (elements) => {
        let arr = []
        elements.forEach(el => {
          arr.push(el.innerText.split('：')[1])
        })
        return arr
      })
      let bookname = pageObj.title, author = eleList[1], tag = eleList[3], pubtime = eleList[4], bookurl = pageObj.href
      let category = await page.$eval('#mute-category > a', el => el.innerText)
      let brief = await page.$eval('body > section > div.content-wrap > div > article', el => el.innerHTML)
      let sql = 'insert into book (bookname,author,tag,pubtime,bookimg,category,brief,bookurl,download) values (?,?,?,?,?,?,?,?,?)'
      let bookimg = await page.$eval('body > section > div.content-wrap > div > article > div.book-info > div.book-left > div > div.bookpic > img', el => el.getAttribute('src'))
      let result = await sqlQuery(sql, [bookname,author,tag,pubtime,bookimg,category,brief,bookurl,aHref])
      console.log(result)
